...
class VersionControl():
    ...
    def compare_modules(self):
        ...
        Moved are the files that exists in both dirs, haven't been altered, but have been moved
        Renamed are exsiting files, renamed
        Deleted are files that exist in the target but do not exist in the source
        '''
        ...
        for source_file in source_not_found.keys():
            for target_file in target_dict.keys():
                if source_dict[source_file] == target_dict[target_file]:
                    ...
                    #if both source and target files have the same dir then its just a rename
